...
const MarketLeaders = ({ stocks }) => {
  ...
  return (
    <div>
      <h1 className="mt-20 text-2xl lg:text-3xl pl-14 lg:pl-20">
        Market Leaders: Today's Top Trade
      </h1>

      <div className="relative flex items-center py-20 px-5 ">
        <MdChevronLeft
          className="opacity-50 cursor-pointer rounded-full bg-gray-800 hover:opacity-100"
          onClick={() => slide("left")}
          size={40}
        />
        ...
        >
          {stocks.map((item) => (
            <Link
              to={`/stock`}
              key={item.id}
              className="w-[260px] bg-[#091620a2] group hover:bg-gradient-to-t from-black to-[#0510a5] border border-gray-500  mx-2 h-full py-5 rounded-md inline-block p-2 cursor-pointer gap-3 ease-in-out duration-300 "
            >
